...
class CamundaRFWorker:
    ...
    def _start_robot_framework_container(self,topic,task_id):
        ...
        try:
            ...
            #entry_point =["/bin/sh", "-c", f"echo hi && cd /tmp/{self.project_name} && {robot_cmd}"]
            print('entry point: ',entry_point)
            result = self.docker_client.containers.run(self.robot_container, network=self.container_network, volumes=self.creds_volume_mount, entrypoint=entry_point, detach=True, auto_remove=True)
    ...
    def _fetch_git_repository_for_task(self,task_id):
        ...
        try:
            ...
            print(self.camunda_engine_url+"/variable-instance?&variableName="+self.git_repo_param)
            print(r.json)
            r.raise_for_status()
            for t in r.json():
